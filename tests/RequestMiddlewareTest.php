...
use Larke\Auth\Middlewares\RequestMiddleware;
...
class RequestMiddlewareTest extends TestCase
{
    ...
    {
        parent::initConfig();
        $this->app['config']->set('larkeauth.basic.model.config_type', 'text');
        $text = <<<'EOT'
[request_definition]
r = sub, obj, act
...
EOT;
        $this->app['config']->set('larkeauth.basic.model.config_text', $text);
    }
